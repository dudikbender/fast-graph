...
async def create_node(label:str, node_attributes: dict, permission: Optional[str] = None, 
    ...
    if label == 'User':
        raise HTTPException(
            ...
            detail=f"Operation not permitted, cannot create a User with this method.",
...
async def update_node(node_id: int, attributes: dict):
    ...
    for key in attributes:
        if key in base_properties:
            raise HTTPException(
                status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
                detail=f"Operation not permitted, that property field cannot be updated.",
                headers={"WWW-Authenticate": "Bearer"})
...
async def delete_node(node_id: int):
    ...
    if not node_data:
        return {'response': f'Node with ID: {node_id} was successfully deleted from the graph.'}
...
@router.get('/read/{relationship_id}', response_model=Relationship)
...
# READ data about a relationship
@router.put('/update/{relationship_id}', response_model=Relationship)
...
async def delete_relationship(relationship_id: int):
    ...
    if not relationship_data:
        return {'response': f'Relationship with ID: {relationship_id} was successfully deleted from the graph.'}
